...
def get_general_parser():
    ...
    parser.add_argument('--language-models',
                        '--lm',
                        dest='models',
    ...
    parser.add_argument("--common-vocab-filename",
                        "--cvf",
...
def get_eval_generation_parser():
    ...
    parser.add_argument('--text', '--t', dest='text', help='text to compute the generation for')
    parser.add_argument('--split_sentence',
                        dest='split_sentence',
                        action='store_true',
...
def get_eval_KB_completion_parser():
    parser = get_general_parser()
    parser.add_argument('--dataset-filename',
                        '--df',
...
def __add_fairseq_args(parser):
    group = parser.add_argument_group('fairseq')
    group.add_argument(
        '--fairseq-model-dir',
        '--fmd',
        dest='data',
        help='directory that contains the fairseq pre-trained model and the vocabulary')
    group.add_argument('--fairseq-model-name',
                       '--fmn',
                       dest='fairseq_model_name',
                       default='wiki103.pt',
                       help="name of the fairseq pre-trained model (default = 'wiki103.pt')")
    group.add_argument('--fairseq-task',
                       '--task',
                       dest='task',
                       default='language_modeling',
                       help="task")
    group.add_argument('--cpu', action='store_true', help='generate on CPU')
    group.add_argument('--output-dictionary-size',
                       type=int,
                       default=-1,
                       help='output dictionary size (default = -1)')
    return group


def __add_bert_args(parser):
    group = parser.add_argument_group('BERT')
    group.add_argument('--bert-model-dir',
                       '--bmd',
                       dest='bert_model_dir',
                       help="directory that contains the BERT pre-trained model and the vocabulary")
    group.add_argument('--bert-model-name',
                       '--bmn',
...
def __add_gpt_args(parser):
    group = parser.add_argument_group('XLM')
    group.add_argument('--xlm-model-path',
                       '--xmp',
                       dest='xlm_model_path',
                       help="Path to the XLM pre-trained model")
    return group


def __add_transformerxl_args(parser):
    group = parser.add_argument_group('GPT')
    group.add_argument('--transformerxl-model-dir',
                       '--tmd',
                       help="directory that contains the pre-trained model and the vocabulary")
    group.add_argument('--transformerxl-model-name',
                       '--tmn',
...
def __add_elmo_args(parser):
    ...
    group.add_argument('--elmo-model-dir',
                       '--emd',
                       dest='elmo_model_dir',
    ...
    group.add_argument('--elmo-warm-up-cycles',
...
def parse_args(parser):
    ...
    if 'fairseq' in args.models_names:
        if args.data is None:
            raise ValueError(
                "to use fairseq you should specify the directory that contains "
                "the pre-trained model and the vocabulary with the option --fairseq-model-dir/--fmd\n"
                "you can also specify the fairseq model name with the option --fairseq-model-name/--fmn (default = 'wiki103.pt')\n"
                "the vocabulary should be in the provided fairseq-model-dir and be named dict.txt")
